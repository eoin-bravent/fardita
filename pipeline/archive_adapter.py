...
DATA_DIR = os.path.join(HERE, "data")    # checked-in inputs: agencies.json, per-reg dates files
CACHE_DIR = os.path.join(HERE, "cache")  # derived + regenerable (gitignored): era surveys, hints
os.makedirs(CACHE_DIR, exist_ok=True)
sys.path.insert(0, HERE)
sys.path.insert(0, os.path.dirname(HERE))
import extract_json as X          # norm/tok/components, _esc, _window, _range_refs, _group_refs, …
import chunker as CK              # ALT_OPENER/ALT_DATE/PRESCRIBED, roman_to_arabic, sort_key, NUMERIC
from store import Store, HASH_FIELDS, content_hash
...
def _find_end_and_alt(children):
    ...
    for i, ch in enumerate(children):
        if ch.tag != "p":
            continue
        cls = _cls(ch)
        low = X.norm(_text(ch)).lower()
        oc = ("Endofclause" if "Endofclause" in cls else
              "Endofprovision" if "Endofprovision" in cls else "")
        if oc or "end of clause" in low or "end of provision" in low:
            end_text = ("(End of provision)" if "provision" in low else
                        "(End of clause)" if "clause" in low else CK.END_MARKER.get(oc, ""))
            end_el, end_idx = ch, i
            break
    alt = None
...
def _ww_find_end_and_alt(blocks):
    ...
    for i, el in enumerate(blocks):
        ...
        if "(end of clause)" in low or "end of clause" == low.strip("() "):
            end_text, end_idx = "(End of clause)", i; break
        if "(end of provision)" in low or "end of provision" == low.strip("() "):
            end_text, end_idx = "(End of provision)", i; break
    alt_idx = -1
...
def _w1_find_end_and_alt(blocks):
    ...
    for i, el in enumerate(blocks):
        low = _ww_flatten_p(el).lower()
        if "(end of clause)" in low or low.strip("() ") == "end of clause":
            end_text, end_idx = "(End of clause)", i; break
        if "(end of provision)" in low or low.strip("() ") == "end of provision":
            end_text, end_idx = "(End of provision)", i; break
    alt_idx = -1
    for i in range(end_idx + 1 if end_idx >= 0 else 0, len(blocks)):
...
LG_ANCHOR = re.compile(r"(?i)<a\s+(name\s*=\s*(?:\"[^\"]*\"|'[^']*'|[^\s>]+))\s*>")
...
def _lg_sections(body):
    ...
    for el in body.iter():
        ...
        if el.tag in ("p", "div"):
            ...
            if _lg_heading_text(el):
                continue                     # a heading handled above
            cur["blocks"].append(el)
        elif el.tag == "table":
            cur["blocks"].append(el)
...
def _lg_find_end_and_alt(blocks):
    ...
    for i, el in enumerate(blocks):
        low = _ww_flatten_p(el).lower()
        if low.strip("() ") == "end of clause" or "(end of clause)" in low:
            end_text, end_idx = "(End of clause)", i; break
        if low.strip("() ") == "end of provision" or "(end of provision)" in low:
            end_text, end_idx = "(End of provision)", i; break
    alt_idx = -1
    for i in range(end_idx + 1 if end_idx >= 0 else 0, len(blocks)):
        if blocks[i].tag == "p" and CK.ALT_OPENER.match(_ww_flatten_p(blocks[i])):
