...
# standalone-CLI source dir: repo-root dita/ snapshot (this module moved into pipeline/;
# the pipeline itself only imports the helpers, which never touch DITA)
DITA = next((p for p in (os.path.join(HERE, "dita"),
                         os.path.join(os.path.dirname(HERE), "dita")) if os.path.isdir(p)),
            os.path.join(HERE, "dita"))
...
# Cross-reference href -> citation, general across agencies + eras. ditaot hrefs look like
# 'FAR_Subpart_5_2', 'Subpart_1901_4_T48_6052522', 'Section_1801_105_2_T48_...', 'FAR_Part_42',
# or a bare numeric run 'FAR_6_302_2'. FAR uses a 'FAR_' prefix and no id suffix; supplements
# carry an agency prefix and/or a ditaot '_T<n>_<id>' suffix, which the FAR-only original left
# as a raw fragment. Returns a citation ('subpart 5.2' / 'part 42' / '6.302-2'); an unresolvable
# opaque id (e.g. 'SSQFWIXN') or a cross-regulation ref (2+ alpha prefixes) is returned unchanged
# so the caller can see it did NOT resolve to a same-regulation citation.
_HREF_ID_SUFFIX = re.compile(r"_T\d+_\d+$")                       # ditaot autogen id: _T48_6052522
_HREF_PARENS = re.compile(r"((?:\([A-Za-z0-9]+\))+)$")           # trailing (a)(2) qualifier glued to the frag
_HREF_KEYWORD = re.compile(r"(?:^|[_-])(Subpart|Part|Section)_(.+)$", re.I)

# Chunk-time {element @id -> section citation} map, for resolving opaque ditaot hrefs ('#SSQFWIXN',
# '#i1495044') that carry no citation structure. Populated by the chunker's pre-pass (build_id_map)
# and consulted only as a FALLBACK, so structural resolution (Subpart_/numeric) always wins. Empty
# outside a chunking run; process-global (build --parallel is process-level, so no cross-talk).
_ID_MAP = {}
...
def _structural_citation(frag):
    """Parse a ditaot fragment (trailing parens peeled, id-suffix stripped) into a same-regulation
    citation, or None when it has no recoverable numeric structure (an opaque id / cross-reg)."""
    mk = _HREF_KEYWORD.search(frag)
    if mk:                                                       # Subpart_/Part_/Section_ (any agency prefix)
        kind, nums = mk.group(1).lower(), mk.group(2).split("_")
        if kind == "part":
            return "part " + nums[0]
        if kind == "subpart":                                    # 'Subpart_5_2' or dotted 'SUBPART_212.70'
            sp = ".".join(nums[:2]) if len(nums) >= 2 else nums[0]
            return ("subpart " + sp) if re.match(r"^\d+\.\d+$", sp) else None
        if kind == "section":
            if len(nums) >= 2 and nums[0].isdigit():             # part.section(-subsection...)
                return f"{nums[0]}.{nums[1]}" + ("-" + "-".join(nums[2:]) if len(nums) > 2 else "")
            return nums[0] if re.match(r"^\d+\.\d", nums[0]) else None   # dotted 'Section_512.203'
        return None
    toks = frag.split("_")                                       # bare numeric run, optional agency prefix
    lead = 0
    while lead < len(toks) and not toks[lead][:1].isdigit():     # count leading non-numeric (prefix) tokens
        lead += 1
    if lead <= 1:                                                # 0 or 1 prefix token -> own-regulation citation
        toks = toks[lead:]
        if len(toks) == 1 and re.match(r"^\d+\.\d", toks[0]):    # already dotted, e.g. 'SOFARS_5601.6033'
            return toks[0]
        if len(toks) >= 2 and toks[0].isdigit():
            return f"{toks[0]}.{toks[1]}" + ("-" + "-".join(toks[2:]) if len(toks) > 2 else "")
    return None                                                  # opaque id / cross-regulation

def href_to_citation(href):
    raw = href.split("#")[-1]
    topic = raw.split("/")[0]                                    # drop an intra-topic /element path (DLAD '#1_602-3/p_…')
    frag = topic[:-5] if topic.endswith(".dita") else topic      # filename-style hrefs, e.g. '4.703.dita'
    parens = ""
    mp = _HREF_PARENS.search(frag)                              # peel a trailing (a)(2) BEFORE stripping the id
    if mp:
        parens, frag = mp.group(1), frag[:mp.start()]
    frag_noid = _HREF_ID_SUFFIX.sub("", frag)                    # drop the ditaot _T48_<id> suffix
    res = _structural_citation(frag_noid)
    if res is not None:
        return res + parens
    for key in (topic, frag, frag_noid, raw, raw.split("/")[-1]):   # opaque id -> its section (topic or element id)
        if key in _ID_MAP:
            return _ID_MAP[key] + parens
    return frag_noid + parens                                    # unresolvable (dangling / non-section anchor) -> raw
