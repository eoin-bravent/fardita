...
Ported into the chunker/ clean-room package from pipeline/store.py. The merge semantics
are VERBATIM (this is the crown jewel — hard-won correctness). Only two things changed:
  * save() no longer relies on any timestamped .bak litter (the old archive backfill DRIVER
    wrote those, never the Store); it offers a single rolling `<name>.prev.json` (opt-in).
  * the store path/naming is standardized to stores/<AG>/store.json (no <AG>_ prefix, no FAR
    pipeline/store special-case), and the filename is parameterizable (`name=`) so the
    companion store (companion.json) reuses this class in the same directory.
...
are the same code path.  Decision table per identity:
    ...
    in snapshot, row in force at D, differs,
        row starts exactly at D                    -> ERRATA: replace content in place,
                                                      old copy appended to <name>_errata.json
...
class Store:
    ...
    `name` selects the file in the agency dir: "store" -> store.json (the regulation),
    "companion" -> companion.json (the companion-doc store). Both share state.json.
    """

    def __init__(self, store_dir, regulation="FAR", *, name="store"):
        self.dir = os.path.abspath(store_dir)
        self.regulation = regulation
        self.name = name
        os.makedirs(self.dir, exist_ok=True)
        self.path = os.path.join(self.dir, f"{name}.json")
        self.errata_path = os.path.join(
            self.dir, "errata.json" if name == "store" else f"{name}_errata.json")
        self.prev_path = os.path.join(self.dir, f"{name}.prev.json")
    ...
    # ---------- persistence ----------
    def save(self, keep_prev=False):
        """Persist the store atomically. NO timestamped .bak litter. When keep_prev is
        True the prior on-disk copy is rolled to `<name>.prev.json` before overwriting
        (the single rolling backup for the LIVE store); a from-scratch rebuild into
        staging leaves it off -- the staging-then-swap discipline is the safety there."""
        self.rows.sort(key=lambda r: (r["citation"], r.get("alternate", ""), r["effective_from"]))
        self.editions.sort(key=lambda e: e["effective_date"])
        if keep_prev and os.path.exists(self.path):
            try:
    ...
    def merge_snapshot(self, snap_rows, effective_date, source_version,
        ...
        def make_row(src_chunk, ef, et):
            ...
            r["commit_date"] = commit_date   # git publish date, denormalized onto the chunk
        ...
        # effective_date is the LEGAL date the version is in force (the temporal key);
        # commit_date is provenance -- when GSA published it to the repo (may lag the
        # effective date). Both are kept so the timeline is legally correct AND the
        # source freshness/provenance is recoverable.
        entry = {"effective_date": D, "source_version": source_version, "source": source,
                 "source_commit": source_commit, "commit_date": commit_date,
                 "ingested_at": now, "stats": {k: stats[k] for k in STAT_KEYS}}
